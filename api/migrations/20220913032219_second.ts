...
export async function up(knex: Knex): Promise<void> {
	...
	await knex.schema.createTable(dietitianReportTableName, (table) => {
		...
		table.float('height').notNullable()
		table.float('weight').notNullable()
		table.float('sys_blood_pressure').notNullable()
		table.float('dia_blood_pressure').notNullable()
		table.float('blood_glucose').notNullable()
		table.integer('follow_up_status').unsigned()
		table.foreign('follow_up_status').references('follow_up.id')
		table.timestamps(false, true)
	})
