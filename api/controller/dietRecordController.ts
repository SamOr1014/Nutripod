...
export class DietRecordController {
	...
	getExercisesByID = async (req: Request, res: Response) => {
		try {
			...
			const yesterdayExercises =
				await this.dietRecordService.getExerciseByID(
					uid,
			...
			if (
				...
				res.status(200).json({
					hasExercises: false,
					message: 'no exercise on both days'
				})
				return
			}
			...
			if (todayExercises.length > 0) {
				...
				for (let exercise of todayExercises) {
					todayTotalCalories +=
						(exercise.duration / 60) * exercise.ex_calories
				}

				if (yesterdayExercises.length > 0) {
					for (let exercise of yesterdayExercises) {
						yesterdayTotalCalories +=
							(exercise.duration / 60) * exercise.ex_calories
					}
					let floorTodayTotalCalories = Math.floor(todayTotalCalories)
					let floorYesterdayTotalCalories = Math.floor(
						yesterdayTotalCalories
					)
					difference =
						((floorTodayTotalCalories -
							floorYesterdayTotalCalories) /
							floorYesterdayTotalCalories) *
						100

					res.status(200).json({
						hasExercises: true,
						message: 'had exercises',
						rate: Math.round(difference),
						todayCalories: floorTodayTotalCalories,
				...
				res.status(200).json({
					hasExercises: true,
					message: 'had exercises',
					todayCalories: Math.floor(todayTotalCalories),
					exercise: todayExercises
				})
			}
		} catch (e) {
	...
	getMonthlyExercises = async (req: Request, res: Response) => {
		try {
			...
			const thisMonthExercises =
				await this.dietRecordService.getMonthlyExercisesByID(
					uid,
					formattedDate.start,
					formattedDate.end
				)

			const lastMonthExercises =
				await this.dietRecordService.getMonthlyExercisesByID(
					uid,
					formattedLastDate.start,
					formattedLastDate.end
				)

			if (
				lastMonthExercises.length === 0 &&
				thisMonthExercises.length === 0
			) {
				res.status(200).json({
					is_exercised: false,
					message: 'No exercises in 2 months!'
				})
				return
			}
			...
			if (thisMonthExercises.length === 0) {
				res.status(200).json({
					is_exercised: false,
					message: 'No exercises in this months!'
				})
				return
			}
			...
			if (thisMonthExercises.length > 0) {
				...
				if (lastMonthExercises.length > 0) {
					...
					res.status(200).json({
						is_exercised: true,
						message: 'had exercises in both months',
						rate: Math.round(difference),
						averageCalories: Math.round(
							thisMonthTotalCalories / today
						)
					})
					return
				}
				res.status(200).json({
					is_exercised: true,
					message: 'Only exercise in this month',
	...
	getDailyInTakeByID = async (req: Request, res: Response) => {
		try {
			...
			const todayIntake = await this.dietRecordService.getFoodIntakeByID(
				uid,
				formattedDate
			)
			const yesterdayIntake =
				await this.dietRecordService.getFoodIntakeByID(
					uid,
					formattedPreviousDate
				)
			...
			if (todayIntake.length === 0 && yesterdayIntake.length === 0) {
				res.status(200).json({
					inTake: false,
					message: 'No Intake for two days'
				})
				return
			}
...
<<<<<<< HEAD
					todayIntakeCalories +=
						intake.food_calories * intake.food_amount
=======
					console.log(intake)
					todayIntakeCalories += intake.food_calories * intake.food_amount
>>>>>>> bfce4b259639a1741a553eb3d84b66a0aa2131be
				}
				if (yesterdayIntake.length > 0) {
					for (let intake of yesterdayIntake) {
						yesterdayIntakeCalories +=
							intake.food_calories * intake.food_amount
					}
					difference =
						((todayIntakeCalories - yesterdayIntakeCalories) /
							yesterdayIntakeCalories) *
						100

					res.status(200).json({
						inTake: true,
						message: 'Intake for two days',
						rate: Math.round(difference),
						intakeCalories: todayIntakeCalories / 100,
						diet: todayIntake
					})
					return
				}
	...
	getMonthlyInTakeByID = async (req: Request, res: Response) => {
		try {
			...
			const thisMonthIntake =
				await this.dietRecordService.getFoodMonthlyIntakeByID(
					uid,
					formattedDate.start,
					formattedDate.end
				)

			const lastMonthIntake =
				await this.dietRecordService.getFoodMonthlyIntakeByID(
					uid,
					formattedLastDate.start,
					formattedLastDate.end
			...
			if (thisMonthIntake.length === 0) {
				res.status(200).json({
					inTake: false,
					message: 'No intake in this month!'
				})
				return
			}
			...
			if (thisMonthIntake.length > 0) {
				...
				if (lastMonthIntake.length > 0) {
					...
					res.status(200).json({
						inTake: true,
						message: 'Have intake in both months',
						rate: Math.round(difference),
						thisMonthIntake: Math.round(
							thisMonthAverage / 100 / today
						)
					})
				}
				res.status(200).json({
					inTake: true,
					message: 'intake for this month only',
					thisMonthIntake: Math.round(
						thisMonthFoodIntake / 100 / today
					)
				})
			}
		} catch (e) {
	...
	addExercise = async (req: Request, res: Response) => {
		try {
			...
			const result = await this.dietRecordService.addExercise(
				uid,
				formattedDate,
				exercise,
				duration
