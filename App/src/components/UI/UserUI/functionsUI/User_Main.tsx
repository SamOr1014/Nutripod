...
export default function UserMain() {
  ...
  const {
    ...
    onOpen: breakfastOnOpen,
    onClose: breakfastOnClose,
  } = useDisclosure();
  const {
    isOpen: lunchOpen,
    onOpen: lunchOnOpen,
  ...
  const {
    isOpen: exerciseOpen,
    onOpen: exercisesOnOpen,
    onClose: exerciseOnClose,
  } = useDisclosure();
  const {
    isOpen: exerciseFormOpen,
    onOpen: exerciseFormOnOpen,
    onClose: exerciseFormOnClose,
  } = useDisclosure();
  const {
    isOpen: foodFormOpen,
    onOpen: foodFormOnOpen,
    onClose: foodFormOnClose,
  } = useDisclosure();
  ...
  async function fetchExercisesFromServer() {
    ...
    axios
      .get(
        `${REACT_APP_API_SERVER}/diet/exercises/${
          userInfo.id
      ...
      .then(({ data }) => {
        if (!data.hasExercises && data.message === "no exercise on both days") {
          ...
          !data.hasExercises &&
          data.message === "No exercise for today"
        ) {
          setCalories(0);
          setRate(0);
  ...
  async function fetchMonthlyExercisesFromServer() {
    ...
    axios
      .get(
        `${REACT_APP_API_SERVER}/diet/monthlyExercises/${
          userInfo.id
        }/${selectedDate?.toISOString()}`,
        {
          headers: {
      ...
      .then(({ data }) => {
        if (
          !data.is_exercised &&
          data.message === "No exercises in 2 months!"
        ) {
          ...
          !data.is_exercised &&
          data.message === "No exercises in this months!"
        ) {
          setAverageCalories(0);
          setTwoMonthsHasExercises(false);
  ...
  async function fetchIntakeFromServer() {
    ...
    axios
      .get(
        `${REACT_APP_API_SERVER}/diet/dailyDiet/${
          userInfo.id
        }/${selectedDate?.toISOString()}`,
        {
          headers: {
  ...
  async function fetchMonthlyIntakeFromServer() {
    ...
    axios
      .get(
        `${REACT_APP_API_SERVER}/diet/monthlyDiet/${
          userInfo.id
        }/${selectedDate?.toISOString()}`,
        {
          headers: {
      ...
      .then(({ data }) => {
        if (!data.inTake && data.message === "No intake in 2 months!") {
          ...
          !data.inTake &&
          data.message === "No intake in this month!"
        ) {
          setAverageIntake(0);
          setAverageIntakeRate(0);
          setTwoMonthsHasIntake(false);
                  ...
                  <ModalBody>
                    <Formik
                      ...
                      onSubmit={async (values) => {
                        axios
                          .get(
                            `${REACT_APP_API_SERVER}/diet/search/${values.food}`,
                            {
                              headers: {
                                Authorization: `Bearer ${locateToken()}`,
                              },
                          ...
                          .then(({ data }) => {
                            ...
                            for (let searchResult of data.list) {
                              ...
                              setResultFoodList((previousList) => [
                                ...previousList,
                                food,
                              ]);
                            }
                    ...
                    {hasSearchResult ? (
                      <Formik
                        ...
                        onSubmit={async (values, { resetForm }) => {
                          axios
                            .post(
                              `${REACT_APP_API_SERVER}/diet/foodIntake`,
                              {
                                values,
                              },
                              {
                      ...
                      >
                        {({ handleSubmit, errors, touched }) => (
                          <Form onSubmit={handleSubmit}>
                            ...
                            <Center justifyContent="space-around" mt={10}>
                              <Button colorScheme="blue" mr={3} type="submit">
                                提交
                              </Button>
                              <Button
          ...
          <Center flex={1} justifyContent={"center"}>
            <StatGroup flex={"1 1 0%"}>
              ...
              <Stat textAlign={"center"}>
                ...
                <StatNumber>
                  {averageIntake > 0 ? `${averageIntake}kcal` : "沒有記錄"}
                </StatNumber>
                <StatHelpText>
                        ...
                        >
                          {({ handleSubmit, errors, touched }) => (
                            <Form onSubmit={handleSubmit}>
                              ...
                              <Center justifyContent="space-around" mt={3}>
                                <Button colorScheme="blue" mr={3} type="submit">
                                  提交
                                </Button>
                                <Button
          ...
          <Center flex={1} justifyContent={"center"}>
            <StatGroup flex={"1 1 0%"}>
              <Stat textAlign={"center"}>
                ...
                <StatNumber>
                  {calories > 0 ? `${calories}Kcal` : "今天還沒有運動"}
                </StatNumber>
              ...
              <Stat textAlign={"center"}>
                ...
                <StatHelpText>
                  ...
                  {twoMonthsHasExercises ? (
                    <StatArrow type={monthRate > 0 ? "increase" : "decrease"} />
